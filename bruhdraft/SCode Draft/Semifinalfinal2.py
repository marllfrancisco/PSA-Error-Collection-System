...
def verify_user_n_passcode():
    ...
    # Hardcoded System Administrator Layer Access Check
...
def initiate_forgot_password_sequence():
    ...
    # Generate 6 Digit Numeric Token Sequence
    ...
    # --- ADD THIS LOGGING POPUP LINE FOR TESTING ---
...
def finalize_password_override(target_username):
    ...
    # Commit execution update routine variables safely
    account_database[target_username][1] = pwd1
    record_accounts()
    
    # --- COMMENT OUT OR REMOVE THIS CLEAN-UP BLOCK ---
    # if target_username in otp_requests_database:
    #     del otp_requests_database[target_username]
    # --------------------------------------------------
    
    # If you implemented "Option 2" (Persistent JSON File for OTPs) in the last step,
    # you should also make sure NOT to call record_otps() here so it stays in the JSON file.
...
def forget_screen():
    ...
    btn_reset = LiftedRoundedButton(container, text="Next Step", image=None, command=initiate_forgot_password_sequence, variant="primary", width=350, height=45)
...
def otp_verification_screen(target_username):
    ...
    btn_verify = LiftedRoundedButton(container, text="Verify Verification Token", image=None, command=lambda: execute_otp_validation_check(target_username), variant="primary", width=350, height=45)
...
def reset_password_entry_screen(target_username):
    ...
    btn_finalize = LiftedRoundedButton(container, text="Finalize Password Changes", image=None, command=lambda: finalize_password_override(target_username), variant="primary", width=350, height=45)
...
def accessibility_screen():
    ...
    lbl3 = tb.Label(container, text="Data Views Text Size", font=("Helvetica", 16, "bold"), foreground=heading_fg)
...
# ==========================================
# SYSTEM WORKSPACE AUDIT LOG PAGES
# ==========================================
class AuditLogPage(tb.Frame):
    def __init__(self, parent, controller, show_enter_new=False):
        ...
        tb.Label(headerframe, text="System Discrepancy Audit Logs", font=titlefont).pack(anchor="w")
        tb.Label(headerframe, text="Active records written by legal verification staff", font=ourfont).pack(anchor="w")
        ...
        tb.Label(utility_frame, text="🔍 Search Data: ", font=("Helvetica", 11)).pack(side=LEFT)
...
# ==========================================
# EMPLOYEE LOGS SYSTEM OBJECT (ADMIN SIDE)
# ==========================================
class EmployeeLogsPage(tb.Frame):
    def __init__(self, parent):
        ...
        tb.Label(headerframe, text="Employee Operational Audit Logs", font=titlefont).pack(anchor="w")
        tb.Label(headerframe, text="Tracks lifecycle updates including registrations and security credential resets", font=ourfont).pack(anchor="w")
        
        utility_frame = tb.Frame(headerframe)
        utility_frame.pack(fill="x", pady=(10, 0))

        tb.Button(utility_frame, text="Refresh Logs", bootstyle="outline-secondary", command=self.refresh_table).pack(side=LEFT, padx=(0, 20))
    ...
    def refresh_table(self):
        ...
        self.dv.delete_rows()
        self.dv.build_table_data(c_employee_logs_headers, formatted_dataset)
        self.dv.load_table_data()
        self.dv.autofit_columns()
        self.dv.autoalign_columns()
...
def call_employee_logs_view():
    global current_active_page_ref
    page_instance = EmployeeLogsPage(content_frame)
    page_instance.pack(fill="both", expand=True)
    current_active_page_ref = page_instance
    apply_font_sizes()

# ==========================================
# OTP REQUESTS SYSTEM OBJECT (ADMIN SIDE)
# ==========================================
class RequestsPage(tb.Frame):
    def __init__(self, parent):
        super().__init__(parent)
        titlefont = ("Helvetica", 18, "bold")
        ourfont = ("Helvetica", 12)
        ...
        tb.Label(headerframe, text="Pending Reset Token Requests", font=titlefont).pack(anchor="w")
        tb.Label(headerframe, text="Active valid 6-Digit OTP security tokens requested by employees", font=ourfont).pack(anchor="w")
        
        utility_frame = tb.Frame(headerframe)
        utility_frame.pack(fill="x", pady=(10, 0))

        tb.Button(utility_frame, text="Refresh Queue Table", bootstyle="outline-secondary", command=self.refresh_table).pack(side=LEFT, padx=(0, 20))
    ...
    def refresh_table(self):
        formatted_dataset = []
        for username, payload in otp_requests_database.items():
            ts = payload["timestamp"].strftime("%Y-%m-%d %H:%M:%S")
            formatted_dataset.append([username, payload["otp"], ts])
            
        self.dv.delete_rows()
        self.dv.build_table_data(c_requests_headers, formatted_dataset)
        self.dv.load_table_data()
        self.dv.autofit_columns()
        self.dv.autoalign_columns()
...
def call_requests_view():
    global current_active_page_ref
    page_instance = RequestsPage(content_frame)
    page_instance.pack(fill="both", expand=True)
    current_active_page_ref = page_instance
    apply_font_sizes()

# ==========================================
# DYNAMIC AUTHORIZED CONTROL MAIN MENU GRID
# ==========================================
def main_menu_screen():
    ...
    main_container = tb.Frame(content_frame, padding=40)
    main_container.pack(expand=True, fill="both")

    # --- ARCHITECTURAL ROUTING TREE FOR AUTHORIZED ROLES ---
    if CURRENT_LOGGED_IN_USER == "admin":
        # 2x2 Clean Uniform Grid Block Implementation Strategy
        main_container.columnconfigure(0, weight=1, uniform="grid_group")
        main_container.columnconfigure(1, weight=1, uniform="grid_group")
        main_container.rowconfigure(0, weight=1, uniform="grid_row")
        main_container.rowconfigure(1, weight=1, uniform="grid_row")

        btn_create = LiftedRoundedButton(
            main_container, text="\nNew Discrepancy Entry", image=plus_icon, compound="top",
            command=lambda: navigate_to(entry_system_screen), width=280, height=140, variant="default", text_size=16
        )
        btn_create.grid(row=0, column=0, sticky="nsew", padx=15, pady=15)

        btn_logs = LiftedRoundedButton(
            main_container, text="\nDiscrepancy Audit Logs", image=paper_icon, compound="top",
            command=lambda: navigate_to(call_audit_logs_view, show_enter_new=False), width=280, height=140, variant="default", text_size=16
        )
        btn_logs.grid(row=0, column=1, sticky="nsew", padx=15, pady=15)

        btn_emp_logs = LiftedRoundedButton(
            main_container, text="\nEmployee Activity Logs", image=gear_icon, compound="top",
            command=lambda: navigate_to(call_employee_logs_view), width=280, height=140, variant="default", text_size=16
        )
        btn_emp_logs.grid(row=1, column=0, sticky="nsew", padx=15, pady=15)

        # Dynamic Grid Allocation for Security Panel Controls
        sub_grid_container = tb.Frame(main_container)
        sub_grid_container.grid(row=1, column=1, sticky="nsew")
        sub_grid_container.columnconfigure(0, weight=1)
        sub_grid_container.rowconfigure(0, weight=1, uniform="sub_rows")
        sub_grid_container.rowconfigure(1, weight=1, uniform="sub_rows")

        btn_new_acc = LiftedRoundedButton(
            sub_grid_container, text="     Create New Account Profile", image=None, compound="left",
            command=lambda: navigate_to(signup_screen), width=280, height=65, variant="primary", text_size=13
        )
        btn_new_acc.grid(row=0, column=0, sticky="new", padx=15, pady=5)

        btn_req_panel = LiftedRoundedButton(
            sub_grid_container, text="     Manage Reset Requests Queue", image=None, compound="left",
            command=lambda: navigate_to(call_requests_view), width=280, height=65, variant="accent", text_size=13
        )
        btn_req_panel.grid(row=1, column=0, sticky="new", padx=15, pady=5)

    else:
        # Standard Staff Member Terminal Setup Layout Map
